...
final class EdfaPgRestApiClient {
    
    private let urlSession: URLSession = .shared
    var printRequestInfo = true
    
    @discardableResult
    func send(_ dataRequest: EdfaPgDataRequest, callback: @escaping (EdfaPgDataResponse) -> Void) -> URLSessionDataTask {
        let task = urlSession.dataTask(with: dataRequest.request) { [weak self] (data, response, error) in
            ...
            DispatchQueue.main.async {
                callback(EdfaPgDataResponse(data: data, response: response, error: error))
            }
        }
        task.resume()
